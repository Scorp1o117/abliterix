#!/usr/bin/env python3
"""ORBA direct (mergeable) on pocket mean-diff. SFT distill collapsed."""
...
def _dump(payload: dict) -> None:
    ...
    (SCRATCH / "qwen38_orba_sweep.json").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    payload = {
        ...
        "apply": "ORBA direct t24-band",
        "points": [],
    }
    print("loading engine (direct/ORBA)...", flush=True)
    engine = SteeringEngine(cfg)
    cache = torch.load(VECTORS, map_location="cpu", weights_only=False)
    vectors = cache["vectors"]
    benign = cache.get("benign_states")
    print(f"vectors {tuple(vectors.shape)} benign={benign is not None}", flush=True)
    ...
    print(f"HIT orba_o{hit:.2f} — saving merged weights...", flush=True)
    if MERGED.exists():
