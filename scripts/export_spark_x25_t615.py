#!/usr/bin/env python3
"""Merge V30 T615 and save Spark-X2.5-4B-abliterated.

User-requested replacement of T580. T615 is 7/100 @ 3-token KL 0.1468.
"""
...
MERGED = Path("/run/media/s117/OS/Models/Spark-X2.5-4B-abliterated")
STEERING = Path(
    "checkpoints_spark_x25_4b_lora_v30/"
    "--run--media--s117--OS--Models--Spark-X2--5-4B_steering.pt"
)
OUT = Path("logs/spark_x25_t615_export.json")
TRIAL = 615


def _cfg() -> AbliterixConfig:
    os.environ["AX_CONFIG"] = CONFIG
    sys.argv = ["export_t615", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def main() -> None:
    ...
    payload = {
        ...
        "dual_hit_005": False,
        "note": "User chose T615 over T580 for leftover R18. Not dual HIT (7/100 @ KL 0.1468).",
    }
    print(
