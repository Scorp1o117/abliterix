#!/usr/bin/env python3
"""CGA V27: interpolate all-layer global_prompt around 17/100 @ 0.0392.

V26 best under 0.05: t0.50 s1.20 d40 → 17/100 @ KL 0.0392.
s1.00 d40 was 37 @ 0.0302 — 20 openings in 0.009 nats. Interpolate
s=1.22–1.40 plus linear_projection and nearby decision layers.
...
OUT = Path("logs/spark_x25_cga_v27.json")
...
GRID = [
    (1.22, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.25, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.28, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.30, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.32, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.35, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.40, 0.50, 18.0, 40.0, True, "angular", 24),
    (1.20, 0.50, 18.0, 40.0, True, "linear_projection", 24),
    (1.28, 0.50, 18.0, 40.0, True, "linear_projection", 24),
    (1.25, 0.50, 18.0, 40.0, True, "angular", 18),
    (1.25, 0.50, 18.0, 40.0, True, "angular", 30),
]


def _cfg() -> AbliterixConfig:
    os.environ["AX_CONFIG"] = CONFIG
    sys.argv = ["spark_cga_v27", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def _dump(payload: dict) -> None:
    ...
    (SCRATCH / "spark_x25_cga_v27.json").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    payload = {
        ...
        "apply": "CGA V27 all-layer global_prompt interpolate 17@0.039; runtime-only, distill on HIT",
        "points": [],
    }
    print("loading original Spark (CGA V27 all-layer interpolate)...", flush=True)
