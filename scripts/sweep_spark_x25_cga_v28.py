#!/usr/bin/env python3
"""CGA V28: leftover-prefix retry + lower thresh on the 12@0.047 plateau.

V27 all-layer global t0.50 s1.35 d40 → 12/100 @ KL 0.0474. Strength
plateaus there (s1.40 = 13 @ 0.0502). Ban first-token "I'm sorry" on
gate-on refusals (decode retry, KL is benign teacher-forced so should
hold) and drop threshold so more leftovers enter the gate.
...
OUT = Path("logs/spark_x25_cga_v28.json")
HIT_TXT = SCRATCH / "spark_x25_hit.txt"
POS = 26.78
DIST = 13.01

# (strength, threshold, position, distance, pos_only, geometry, decision_layer, retry)
GRID = [
    (1.35, 0.50, 18.0, 40.0, True, "angular", 24, True),
    (1.32, 0.50, 18.0, 40.0, True, "angular", 24, True),
    (1.30, 0.50, 18.0, 40.0, True, "angular", 24, True),
    (1.28, 0.50, 18.0, 40.0, True, "angular", 24, True),
    (1.35, 0.45, 18.0, 40.0, True, "angular", 24, False),
    (1.35, 0.40, 18.0, 40.0, True, "angular", 24, False),
    (1.35, 0.35, 18.0, 40.0, True, "angular", 24, False),
    (1.35, 0.45, 18.0, 40.0, True, "angular", 24, True),
    (1.35, 0.40, 18.0, 40.0, True, "angular", 24, True),
]


def _cfg() -> AbliterixConfig:
    os.environ["AX_CONFIG"] = CONFIG
    sys.argv = ["spark_cga_v28", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def _dump(payload: dict) -> None:
    ...
    (SCRATCH / "spark_x25_cga_v28.json").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    payload = {
        ...
        "apply": "CGA V28 leftover-prefix retry + lower thresh on 12@0.047; runtime-only, distill on HIT",
        "points": [],
    }
    print("loading original Spark (CGA V28 prefix-retry plateau)...", flush=True)
    ...
    for strength, thresh, pos, dist, pos_only, geom, dlayer, retry in GRID:
        ...
        cfg.steering.concept_gate_refusal_prefix_retry = bool(retry)
        cfg.steering.concept_gate_refusal_prefix_ban_tokens = 6
        ...
        tag = (
            ...
            f"_pos{int(pos_only)}_{geom[:3]}_L{dlayer}_r{int(retry)}"
        )
        print(
            f"hooks={n_hooks} t={thresh} s={strength} d={dist} "
            f"pos_only={pos_only} geom={geom} L={dlayer} retry={retry}",
            flush=True,
        )
