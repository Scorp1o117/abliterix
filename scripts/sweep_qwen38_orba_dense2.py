...
OUT = Path("logs/qwen38_orba_dense2.json")
...
def _cfg():
    ...
    sys.argv = ["orba_dense2", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def main():
    ...
    for tag, o, down, dist in jobs:
        ...
        (SCRATCH / "qwen38_orba_dense2.json").write_text(text)
        if row["in_budget"]:
    ...
    payload["finished_utc"] = datetime.now(timezone.utc).isoformat()
    OUT.write_text(json.dumps(payload, indent=2))
    (SCRATCH / "qwen38_orba_dense2.json").write_text(json.dumps(payload, indent=2))
