#!/usr/bin/env python3
"""T32 leftover r2 with flipped sign and r1-orthogonal component.

V11 applied leftover mean-diff as-is and *undid* T32 (57/100 @ 0.0456 →
95/100 @ 0.042). cos(r1, r2)=0.78 so most of r2 is the same hemisphere, but
the LoRA peel still restored refusals. This sweep:
  1. r2_neg = -r2          (sign error hypothesis)
  2. r2_orth = r2 - proj_r1 (true leftover subspace)
Score vs original 3-token KL. HIT: refusals≤10/100 AND KL≤0.05.
"""
...
R2_CACHE = Path("logs/spark_x25_t32_r2.pt")
SCRATCH = Path("/tmp/grok-goal-85e2331705c4/implementer")
OUT = Path("logs/spark_x25_t32_r2flip.json")
...
# (variant, o_max, o_min_frac, q_max)
GRID = [
    ("neg", 0.8, 0.50, 0.0),
    ("neg", 1.6, 0.60, 0.0),
    ("neg", 2.4, 0.70, 0.0),
...
def _cfg() -> AbliterixConfig:
    ...
    sys.argv = ["spark_t32_r2flip", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def _dump(payload: dict) -> None:
    ...
    (SCRATCH / "spark_x25_t32_r2flip.json").write_text(text, encoding="utf-8")


def _profiles(o_max: float, o_min_frac: float, q_max: float) -> dict:
    return {
        "attn.o_proj": SteeringProfile(
            max_weight=float(o_max),
            max_weight_position=T32_O_POS,
            min_weight=max(0.0, float(o_max) * float(o_min_frac)),
...
def _orth(r2: torch.Tensor, r1: torch.Tensor) -> torch.Tensor:
    out = r2.float().clone()
    u = r1.float()
    if u.ndim != 2 or out.ndim != 2:
        raise ValueError(f"expected 2D vectors, got r1={tuple(u.shape)} r2={tuple(out.shape)}")
...
def main() -> None:
    ...
    if R2_CACHE.is_file():
        blob = torch.load(R2_CACHE, map_location="cpu", weights_only=False)
        r2 = blob["r2"]
        beni = blob.get("beni")
        payload["leftover_n"] = blob.get("leftover_n")
        payload["r2_cache"] = str(R2_CACHE)
        print(f"loaded r2 cache {tuple(r2.shape)} leftover_n={payload['leftover_n']}", flush=True)
    else:
        ...
        R2_CACHE.parent.mkdir(parents=True, exist_ok=True)
        torch.save({"r2": r2, "beni": beni, "leftover_n": len(leftover)}, R2_CACHE)
        print(f"cached r2 → {R2_CACHE}", flush=True)
    ...
    r2_neg = (-r2.float()).to(dtype=r2.dtype)
    r2_orth = _orth(r2, v1)
    cos_neg = float(torch.nn.functional.cosine_similarity(v1[i].float(), r2_neg[i].float(), dim=0))
    cos_orth = float(torch.nn.functional.cosine_similarity(v1[i].float(), r2_orth[i].float(), dim=0))
    print(
        f"cos layer~{i}: r2={cos:.4f}  neg={cos_neg:.4f}  orth={cos_orth:.4f}",
        flush=True,
    )
    payload["cos_r1_r2_layer27"] = cos
    payload["cos_r1_r2neg_layer27"] = cos_neg
    payload["cos_r1_r2orth_layer27"] = cos_orth
    _dump(payload)

    variants = {"neg": r2_neg, "orth": r2_orth}
    _merge_t32_keep_engine(engine, rank=3)

    for variant, o_max, o_frac, q_max in GRID:
        engine.restore_baseline()
        apply_steering(
            engine,
            variants[variant],
            None,
            _profiles(o_max, o_frac, q_max),
            cfg,
            benign_states=beni,
        )
        tag = f"t32+r2{variant}_o{o_max:.1f}_q{q_max:.1f}"
        row = _score(tag, scorer, engine)
        row.update({"variant": variant, "r2_o": o_max, "r2_q": q_max})
