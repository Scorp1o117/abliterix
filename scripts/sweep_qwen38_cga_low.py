#!/usr/bin/env python3
"""Low-threshold prompt-latched CGA.

All-layer prompt-latch at 0.20 already isolated benign KL (75/100 @ 0.004).
Token-scope at 0.20–0.50 stayed ~55–67 @ 0.18–0.23. Push the prompt
threshold down so more harmful last-tokens open the gate.
"""
...
OUT = Path("logs/qwen38_cga_low_sweep.json")


def _load_cfg() -> AbliterixConfig:
    os.environ["AX_CONFIG"] = CONFIG
    sys.argv = ["sweep_cga_low", "--config", CONFIG, "--seed", "117"]
    cfg = AbliterixConfig()
...
def _dump(payload: dict) -> None:
    ...
    (SCRATCH / "qwen38_cga_low_sweep.json").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    payload = {
        ...
        "apply": "all-layer prompt/global CGA at low thresholds",
        "points": [],
    }
    ...
    scorers = blob["scorers"] if isinstance(blob, dict) and "scorers" in blob else blob
    engine._concept_scorers = scorers
    print(f"scorers={len(scorers)} vectors={tuple(vectors.shape)}", flush=True)

    # global t0.35 / strength 1.0 already scored 20/100 @ 0.040.
    jobs = [
        ("global_prompt", 0.25, 1.00),
        ("global_prompt", 0.28, 1.00),
        ("global_prompt", 0.20, 1.00),
        ("global_prompt", 0.32, 1.00),
        ("global_prompt", 0.35, 1.30),
        ("global_prompt", 0.35, 1.60),
        ("global_prompt", 0.15, 1.00),
        ("prompt", 0.08, 1.00),
    ]
    ...
    for scope, thresh, strength in jobs:
        ...
        tag = f"cga_low_{scope}_t{thresh:.2f}_{strength:.2f}"
