#!/bin/bash
# Ling-3.0-flash v35: canonical full quality/compliance evaluation
...
for suffix in _baseline.pt _steering.pt _generated_gate_responses.pt; do
  for f in checkpoints_ling30_flash_v34_canonical_batching/*"$suffix"; do
    [[ -f "$f" ]] || continue
    bn=$(basename "$f")
