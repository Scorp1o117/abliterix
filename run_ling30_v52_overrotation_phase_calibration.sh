#!/bin/bash
# Ling-3.0-flash v52: isolate prefill vs decode angular over-rotation
...
SOURCE_CKPT="checkpoints_ling30_flash_v51_overrotation_calibration"
...
V52_OPTIMIZATION_JSON="{\"num_trials\":3,\"num_warmup_trials\":3,\"checkpoint_dir\":\"$CKPT\",\"refusal_prescreen_enabled\":true,\"refusal_prescreen_size\":60,\"refusal_prescreen_pass_max\":0,\"refusal_prescreen_prune_min\":0,\"refusal_prescreen_seed\":117,\"validation_kl_enabled\":false,\"generation_health_enabled\":false,\"thinking_leak_detection_enabled\":false,\"seed_trials\":[{\"concept_gate_angular_overrotation_phase\":\"prefill\",$COMMON},{\"concept_gate_angular_overrotation_phase\":\"decode\",$COMMON},{\"concept_gate_angular_overrotation_phase\":\"all\",$COMMON}]}"

echo "=== Ling-3.0-flash v52 ANGULAR OVER-ROTATION PHASE CALIBRATION ==="
echo "  calibration: target train[800:900], fixed canonical 60 subset"
echo "  trials: max 1.20 over-rotation during prefill / decode / all"
echo "  guard: formal target train[900:] is not used"

exec abliterix \
  --config "$CONFIG" \
  --seed 117 \
  --optimization "$V52_OPTIMIZATION_JSON" \
  ...
  --steering.search-concept-gate-angular-overrotation-phase \
