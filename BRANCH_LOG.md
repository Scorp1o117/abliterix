...
| 本日志最近更新 | 2026-09-11 (**交付物改为 v8-t17**：主人裁定 KL 优先) |

## -1cx. 2026-09-11 — 交付物裁定：采用 v8-t17（KL 优先）

**主人裁定**：「用 t17，拒绝率差不多的情况下肯定选低 KL 的」——即 **KL 是首要质量门**，
拒绝数在同一档内（9 vs 12）时以 KL 定胜负。这不推翻 §-1cw 的记录，而是明确了选点准则，
**优先级写在这里以免以后再犹豫**：

> 选点顺序：① 拒绝 ≤10 硬门槛（破线即出局）；② 满足 ① 的候选里 **KL 最小者胜**；
> ③ KL ≤0.05 优先于 ≤0.1，但两者都算合规。

### 最终交付物

| 项 | 值 |
|---|---|
| 路径 | `/run/media/s117/OS/Models/Nex-N2.5-mini-abliterix-v8-t17` |
| 规模 | 21 shards、69.32 GB、693 tensors（index 与磁盘一致 ✓） |
| 配方 | v8 trial 17：direct + EGA，per-layer 向量，含 q/k/v（o 0.728 / down 1.659 / q 0.325 / k 0.666 / v 0.300） |
| 160 口径实测 | **12/100 @ KL 0.0662**（烘焙后） |
| 清单 | `nex25_manifest.json`（21 个 SHA256 + trial_attrs + max_gen_tokens=160） |
| 确定性验证 | 二次烘焙首分片 SHA256 `2020411eb4bd95efac…` 与首次**逐位一致** → 烘焙可复现 |

对照：阶段一 `Nex-N2.5-mini-abliterix` = 9/100 @ KL 0.0845（保留在盘上，未删除）。
按上面的选点准则，**t17 胜出**（KL 低 0.018，拒绝同档）。

---

## -1cw. 2026-09-11 — v8 六十轮收官：搜索未超越已发布模型（阴性结果，已归档）

### 结果（全部 160 口径、**烘焙后实测**）

| 模型 | 拒绝 | KL | 合规（≤10 且 ≤0.1） |
|---|---|---|---|
| **阶段一 `Nex-N2.5-mini-abliterix`（已发布）** | **9** | 0.0845 | ✅ |
| v8-t17（60 轮搜索的合规最优） | 12 | 0.0662 | ❌（拒绝超 1） |

两者**互不支配**：t17 KL 更低但拒绝破线。按「拒绝 ≤10 为硬门槛、KL 为分级质量门」的标准，
**阶段一模型维持交付** ✓

### in-run 与烘焙的偏差是**配方相关**的（重要）

| 配方 | in-run | 烘焙后 |
|---|---|---|
| 0.33×（阶段一） | 18/100 | **9/100** |
| v8-t17 | 12/100 | **12/100** |

即 PeftModel 包装带来的差异**不是统一偏移**，所以**任何候选都必须烘焙后实测**才能下定论
（这次幸好做了，否则会误判 t17 更优）。

### 搜索空间结论（本模型上已探明）

- 变换：standard ≫ orba/biprojected（12 vs 15/19）
- 方向配方：`harmfulness_pair` 在 direct+MoE 上致命不兼容（崩 run）
- 判别层选择：降 27% KL 但拒绝上限 ~17，且 >0.25× 生成崩坏 → 无法过拒绝门槛
- `vector_scope`：TPE 自发探索了 `global`，未产生更优点
- 形状（位置/距离/min-frac/组件配比）60 轮：前沿 `(10,0.0964) … (77,0.0139)`，KL≤0.05 段拒绝 23-77
- 结论：**该模型的拒绝-KL 权衡很陡**，KL 0.05 与拒绝 10 在当前方法族下无法同时满足

### 产物

`Nex-N2.5-mini-abliterix-v8-t17`（21 shards、64.56 GiB、SHA256 清单齐全）已按主人
「测试完记录数据后删除非最终版本」的规则删除；数字全部记录在本节。
