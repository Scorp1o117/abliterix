#!/bin/bash
# Ling-3.0-flash v49: subtract train-holdout failure-conditioned residual
...
SOURCE_CKPT="checkpoints_ling30_flash_v47_failure_conditioned_variants"
...
V49_OPTIMIZATION_JSON="{\"num_trials\":5,\"num_warmup_trials\":5,\"checkpoint_dir\":\"$CKPT\",\"refusal_prescreen_enabled\":true,\"refusal_prescreen_size\":60,\"refusal_prescreen_pass_max\":0,\"refusal_prescreen_prune_min\":0,\"refusal_prescreen_seed\":117,\"validation_kl_enabled\":false,\"generation_health_enabled\":false,\"thinking_leak_detection_enabled\":false,\"seed_trials\":[{\"steering_variant\":\"primary\",$COMMON},{\"steering_variant\":\"failure_alpha_neg_0p125\",$COMMON},{\"steering_variant\":\"failure_alpha_neg_0p25\",$COMMON},{\"steering_variant\":\"failure_alpha_neg_0p5\",$COMMON},{\"steering_variant\":\"failure_alpha_neg_1\",$COMMON}]}"

echo "=== Ling-3.0-flash v49 NEGATIVE FAILURE VARIANTS ==="
echo "  calibration: target train[800:900], same canonical 60 labels as v46"
echo "  variants: bitwise exact primary minus alpha*(failure-success residual)"
echo "  guard: eval[900:] is not used; no prompt text is persisted"

exec abliterix \
  --config "$CONFIG" \
  --seed 117 \
  --optimization "$V49_OPTIMIZATION_JSON" \
