#!/bin/bash
# Ling-3.0-flash v53: norm-preserving angular vs linear projection geometry
...
SOURCE_CKPT="checkpoints_ling30_flash_v52_overrotation_phase_calibration"
...
V53_OPTIMIZATION_JSON="{\"num_trials\":4,\"num_warmup_trials\":4,\"checkpoint_dir\":\"$CKPT\",\"refusal_prescreen_enabled\":true,\"refusal_prescreen_size\":60,\"refusal_prescreen_pass_max\":0,\"refusal_prescreen_prune_min\":0,\"refusal_prescreen_seed\":117,\"validation_kl_enabled\":false,\"generation_health_enabled\":false,\"thinking_leak_detection_enabled\":false,\"seed_trials\":[{\"concept_gate_intervention_geometry\":\"angular\",\"attn.o_proj.max_weight\":1.20,$COMMON},{\"concept_gate_intervention_geometry\":\"linear_projection\",\"attn.o_proj.max_weight\":1.00,$COMMON},{\"concept_gate_intervention_geometry\":\"linear_projection\",\"attn.o_proj.max_weight\":1.20,$COMMON},{\"concept_gate_intervention_geometry\":\"linear_projection\",\"attn.o_proj.max_weight\":1.40,$COMMON}]}"

echo "=== Ling-3.0-flash v53 PROJECTION GEOMETRY CALIBRATION ==="
echo "  calibration: target train[800:900], fixed canonical 60 subset"
echo "  trials: angular 1.20 vs linear projection 1.00/1.20/1.40"
echo "  guard: formal target train[900:] is not used"

exec abliterix \
  --config "$CONFIG" \
  --seed 117 \
  --optimization "$V53_OPTIMIZATION_JSON" \
  ...
  --steering.search-concept-gate-intervention-geometry \
