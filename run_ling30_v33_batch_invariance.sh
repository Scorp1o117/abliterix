#!/bin/bash
# Ling-3.0-flash v33: canonical gate prepass + reverse-order replay
...
for suffix in _baseline.pt _steering.pt _generated_gate_responses.pt; do
  for f in checkpoints_ling30_flash_v32_global_profile_search/*"$suffix"; do
    [[ -f "$f" ]] || continue
    bn=$(basename "$f")
