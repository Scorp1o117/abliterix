#!/bin/bash
# Ling-3.0-flash v38: flat full-depth gated removal canonical probe
...
V38_OPTIMIZATION_JSON='{"num_trials":1,"num_warmup_trials":1,"checkpoint_dir":"checkpoints_ling30_flash_v38_flat_full_depth_probe","refusal_prescreen_enabled":true,"refusal_prescreen_size":60,"refusal_prescreen_pass_max":0,"refusal_prescreen_prune_min":0,"refusal_prescreen_seed":117,"validation_kl_enabled":false,"generation_health_enabled":false,"thinking_leak_detection_enabled":false,"seed_trials":[{"vector_scope":"per layer","vector_index":30.25491805410134,"attn.o_proj.max_weight":1.0,"attn.o_proj.max_weight_position":24.6,"attn.o_proj.min_weight":1.0,"attn.o_proj.min_weight_distance":24.6}]}'

echo "=== Ling-3.0-flash v38 FLAT FULL-DEPTH GATED PROBE ==="
echo "  protocol: canonical gate prepass + canonical batch16"
echo "  intervention: adaptive 90-degree removal across all 42 layers"
echo "  comparison: v36 late-wide profile = 10/60"

exec abliterix \
  --config "$CONFIG" \
  --seed 117 \
  --optimization "$V38_OPTIMIZATION_JSON" \
  --steering.min-weight-frac-max 1.0 \
